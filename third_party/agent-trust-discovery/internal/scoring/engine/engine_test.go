...
func TestEvaluateReproducesWorkedExample(t *testing.T) {
	...
	for _, dim := range []domain.Dimension{domain.DimensionSolvency, domain.DimensionSafety} {
		d := findDim(t, ev, dim)
		if d.Score != 0 || len(d.SignalScores) != 0 {
			t.Errorf("%s should be empty: score=%d signals=%d", dim, d.Score, len(d.SignalScores))
		}
	}
	// Overpass addition (PATCHES.md): behavior carries pass_delivery. With no
	// observation it scores 0 and counts (cold start), so the dimension has one
	// signal at score 0, not zero signals.
	if b := findDim(t, ev, domain.DimensionBehavior); b.Score != 0 || len(b.SignalScores) != 1 {
		t.Errorf("behavior: score=%d signals=%d, want 0/1", b.Score, len(b.SignalScores))
	}
}
