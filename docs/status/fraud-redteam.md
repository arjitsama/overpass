...
|---|---|---|---|---|---|---|
| 1 | payto_binding_check | their agent, 04:56:45 | WARNING "payTo present but absent from signed card extension", `attested_in_signed_card: null`, `card_signed: true` | get_quote x402 challenge, payTo `0x0000…0000` | n/a (structural probe; does not affect battery verdict) | station log 08:56:46Z `python-httpx/0.28.1` tools/call get_quote {MAD,SIN,2026-10-14}; our card carries payTo in `x-payment` and in the signed `supplier-mcp/v1` extension (`params.x402.payTo`), but their parser wants a shape we could not observe: their own supplier's card has **no payTo anywhere** (`payTo@-1`), so the reference target gets the same WARNING. |
